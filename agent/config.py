...
def normalize_config(config, config_directory):
    ...
    fixed_values = {
        ...
        for name, value in config.get("fixed_values", {}).items()
    }

    for name, path in config.get("fixed_value_path", {}).items():
        path = Path(path)

        if not path.is_absolute():
            path = config_directory / path

        fixed_values[name] = np.load(path, allow_pickle=False)

    observed_data_path = config.get("observed_data_path")
    ...
    normalized = {
        ...
        "prior_bounds": {
            name: tuple(bounds)
            for name, bounds in inference_config["prior_bounds"].items()
        },
        "fixed_values": fixed_values,
        "observed_data_path": observed_data_path,
...
def validate_config(config):
    required_sections = {"simulator", "inference", "abc",}
    missing_sections = required_sections - set(config)

    if missing_sections:
        raise ValueError(f"Missing config sections: {sorted(missing_sections)}")

    required_abc_fields = {"epsilon", "n_simulations", "random_seed"}
    missing_abc_fields = (required_abc_fields - set(config["abc"]))

    if missing_abc_fields:
        raise ValueError(f"Missing ABC settings: {sorted(missing_abc_fields)}")
...
def load_config(config_path):
    ...
    normalized_config = normalize_config(raw_config, config_path.parent)

    return raw_config, normalized_config

def create_synthetic_config(output_path, true_parameter_values, generation_seed):
    if not true_parameter_values:
        raise ValueError("True parameter values are required.")
...
def load_synthetic_config(config_path):
    ...
    if config_path.suffix.lower() != ".json":
        raise ValueError("Synthetic config must be a JSON file.")

    with config_path.open("r", encoding="utf-8") as file:
        config = json.load(file)

    true_values = config.get("true_parameter_values")
    generation_seed = config.get("generation_seed")

    if not isinstance(true_values, dict):
        raise ValueError("Synthetic config does not contain valid true parameter values.")

    if not isinstance(generation_seed, int):
        raise ValueError("Synthetic config does not contain a valid generation seed.")
